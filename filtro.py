import cv2
import numpy as np

# Carregar arquivos de configuração e pesos do YOLO
net = cv2.dnn.readNetFromDarknet("yolov3.cfg", "yolov3.weights")
output_layers = net.getUnconnectedOutLayersNames()


# Carregar arquivo com os nomes das classes
classes = []
with open("coco.names", "r") as f:
    classes = f.read().splitlines()

# Inicializar a webcam
cap = cv2.VideoCapture(0)

while True:
    # Capturar o vídeo da webcam
    ret, frame = cap.read()

    # Detectar objetos usando YOLO
    height, width, channels = frame.shape
    blob = cv2.dnn.blobFromImage(
        frame, 0.00392, (416, 416), (0, 0, 0), True, crop=False
    )
    net.setInput(blob)
    outs = net.forward(output_layers)

    # Informações de detecção
    class_ids = []
    confidences = []
    boxes = []

    # Extrair informações de detecção
    for out in outs:
        for detection in out:
            scores = detection[5:]
            class_id = np.argmax(scores)
            confidence = scores[class_id]
            if (
                confidence > 0.5 and classes[class_id] == "pessoa"
            ):  # Filtrar apenas pessoas
                # Coordenadas do bounding box
                center_x = int(detection[0] * width)
                center_y = int(detection[1] * height)
                w = int(detection[2] * width)
                h = int(detection[3] * height)

                # Coordenadas do canto superior esquerdo
                x = int(center_x - w / 2)
                y = int(center_y - h / 2)

                boxes.append([x, y, w, h])
                confidences.append(float(confidence))
                class_ids.append(class_id)

    # NMS (Non-Maximum Suppression)
    indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.5, 0.4)

    # Desenhar bounding boxes para pessoas e exibir
    for i in range(len(boxes)):
        if i in indexes:
            x, y, w, h = boxes[i]
            label = classes[class_ids[i]]
            color = (61, 93, 75)  # Cor do bounding box (verde)
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
            cv2.putText(
                frame, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2
            )

    # Exibir o resultado
    cv2.imshow("Person Detection", frame)

    if cv2.waitKey(1) == 27:  # Pressione ESC para sair
        break

cap.release()
cv2.destroyAllWindows()
